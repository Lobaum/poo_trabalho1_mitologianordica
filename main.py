...
from recompensas.tesouros import AltarDeGelo, Bau, bau_aleatorio
    ...
    if acao == "1":
        if player.energia <= 0:
            ...
            if d6 <= 2:
                ...
                print(f"Vida: {monstro.vida_atual}/{monstro.vida_maxima} - Nível ({monstro.dificuldade})")
                ...
                eventos = random.choice(["bau", "altar"])
                if eventos == "bau":
                    evento = bau_aleatorio()
                    evento.tentar_abrir(player)
                else: 
                    evento = AltarDeGelo()
