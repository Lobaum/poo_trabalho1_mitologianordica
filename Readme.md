# ﹏𓊝﹏ Engine de RPG D20 - Mitologia Nórdica ﹏𓊝﹏

## Trabalho Acadêmico - Programação Orientada a Objetos (POO)

Este projeto foi desenvolvido como parte de um **trabalho acadêmico da faculdade**, com o objetivo de aplicar conceitos de:

- Programação Orientada a Objetos (POO)

- Modelagem de Classes

- Estruturação de Sistemas

- Mecânicas baseadas em RPG D20

---

## 🚀 Tecnologias Utilizadas

### 🐍 **Python**
### 🎲 Sistema baseado em D20
### 🧠 Programação Orientada a Objetos (POO)

---

## 📌 Funcionalidades Implementadas

- 🤠 **Base Jogador**
- 👺 **Base inimigos**

---

## 🎓 Sobre o Projeto

# ⚔️ Classes:

## 🪓 Quebra-Escudos

- Base mitológica: cultura guerreira viking e combate brutal corpo a corpo.

- Função: Dano físico agressivo

- Conceito: especialista em destruir defesa inimiga.

### 📖 Habilidades:

- 🛡 Estilhaçar Defesa | reduz armadura do alvo

- 🪓 Golpe Implacável | dano extra contra inimigos protegidos

- 🩸 Sede de Combate | ganha bônus ao derrotar oponentes

### Características de Classe:

> **Pontos de Vida: 14 PV. +4 PV por nível**

> **Pontos de Energia (PE): 3 por nível**

<!-- > **Perícias: Luta (For) e Atletismo (For), mais 3 à escolha entre: Intimidação (Car), Guerra (Int), Iniciativa (Des), Percepção (Sab), Sobrevivência (Sab), Reflexos (Des)** -->

> **Proficiências: Armas marciais, escudos e armaduras médias/pesadas**

---

## 🔮 Tecelão de Runas

- Base Mitológica: Magia rúnica (Seiðr)
  
- Função: Suporte / Controle
  
- Conceito: Prepara runas no campo antes de ativá-las

#### 📖 Habilidades:

- ᚦ Runa de Prisão | reduz movimento

- ᚨ Runa Vital | regenera aliados

- ᚲ Runa da Ruptura | amplifica dano recebido

`Precisa de 1 turno para a runa antes da ativação.`

### Características de Classe:

> **Pontos de Vida: 10 PV. +3 PV por nível**

> **Pontos de Energia (PE): 5 por nível**

<!-- > **Perícias:
Ocultismo (Int) e Vontade (Sab), mais 3 à escolha entre:
Intuição (Sab), Religião (Sab), Ofício (Int), Percepção (Sab), Diplomacia (Car)** -->

> **Proficiências:
Armas simples, armaduras leves**

---

## 🌫 Caminhante da Névoa

- Base mitológica: reinos como Niflheim, neblina e frio primordial.
  
- Função: Assassino / Controle de terreno
  
- Conceito: luta escondido em névoa e gelo.

#### 📖 Habilidades:

- 🌫 Véu Nebuloso | chance de esquiva aumentada

- 👣 Passo Silencioso | move-se sem provocar ataques de oportunidade

- 🗡 Ataque Invisível | dano crítico se não for detectado

`Se começar o turno oculto → ganha bônus de crítico.`

### Características de Classe:

> **Pontos de Vida: 12 PV. +3 PV por nível`**

> **Pontos de Energia (PE): 4 por nível**

<!-- > **Perícias:
Iniciativa (Des) e Reflexos (Des), mais 4 à escolha entre:
Furtividade (Des), Percepção (Sab), Acrobacia (Des), Intuição (Sab), Sobrevivência (Sab)** -->

> **Proficiências:
Armas leves, armaduras leves**

---

## 🐉 Caçador de Feras

- Base mitológica: criaturas como draugr, trolls e serpentes gigantes.

- Função: Especialista contra monstros

#### 📖 Habilidades:

- 🎯 bônus contra criaturas grandes

- 🧂 resistência a veneno

- 🪤 armadilhas rúnicas

`Ganha bônus baseado no tipo de criatura enfrentada.`

### Características de Classe:

> **Pontos de Vida: 13 PV. +4 PV por nível`**

> **Pontos de Energia (PE): 3 por nível**

<!-- > **Perícias:
Sobrevivência (Sab) e Pontaria (Des), mais 3 à escolha entre:
Percepção (Sab), Atletismo (For), Intuição (Sab), Guerra (Int)** -->

> **Proficiências:
Armas marciais, arcos, armaduras médias**

---

## 🪶 Portador de Presságios

- Base mitológica: profecia e destino (conceito do Wyrd)

- Função: Manipulador de dados (excelente pro sistema D20)

- Conceito: altera probabilidades.

#### 📖 Habilidades:

- 🎲 Alterar Destino | pode forçar rerrolagem

- ⏳ Vislumbre Futuro | vê o próximo resultado antes de decidir agir

- ⚖ Equilíbrio do Wyrd | troca resultado com aliado

### Características de Classe:

> **Pontos de Vida: 9 PV. +3 PV por nível**

> **Pontos de Energia (PE): 6 por nível**

<!-- > **Perícias:
Vontade (Sab) e Intuição (Sab), mais 3 à escolha entre:
Ocultismo (Int), Religião (Sab), Diplomacia (Car), Percepção (Sab)** -->

> **Proficiências:
Armas simples, nenhuma armadura**

---

## ⚔ Forjador

- Base mitológica: importância da forja, armas lendárias, ferreiros.

- Função: Suporte ofensivo

- Conceito: melhora armas em combate.

#### 📖 Habilidades:

- 🔥 Encantar arma por turnos

- 🛠 Reparar  equipamento durante batalha

- 💥 Sobrecarga | arma causa efeito elemental temporário

`Sistema de aprimoramento temporário.`

### Características de Classe:

> **Pontos de Vida: 13 PV. +4 PV por nível**

> **Pontos de Energia (PE): 4 por nível**

<!-- > **Perícias:
Ofício (Int) e Guerra (Int), mais 3 à escolha entre:
Atletismo (For), Intimidação (Car), Percepção (Sab)** -->

> **Proficiências:
Armas marciais, armaduras médias e pesadas**

---

## 🩶 Errante

- Base mitológica: Conexão entre os Nove Mundos por Yggdrasil.

- Função: Híbrido versátil

- Conceito: Alterna estilos de combate conforme a postura ativa.

#### 📖 Habilidades:

- 🌿 Postura das Plantas | bônus em ataques corpo a corpo e testes físicos

- 🔥 Postura das Chamas | ataques causam dano elemental adicional

- ❄ Postura Glacial | reduz velocidade dos inimigos atingidos

- ⚡ Postura Tempestuosa | bônus em iniciativa e chance de crítico

`Troca postura como ação bônus.`

### Características de Classe:

> **Pontos de Vida: 12 PV. +3 PV por nível**

> **Pontos de Energia (PE): 4 por nível**

<!-- > **Perícias:
Iniciativa (Des) e Intuição (Sab), mais 3 à escolha entre:
Atletismo (For), Percepção (Sab), Reflexos (Des), Sobrevivência (Sab), Guerra (Int)** -->

> **Proficiências:
Armas marciais, armaduras médias**

---

## 🌑 Guardião

- Base mitológica: Rituais funerários e espíritos inquietos.

- Função: Tank / Controle

#### 📖 Habilidades:

- 🪦 Aura Sepulcral | reduz ataque inimigo

- ⚰ Vínculo Espiritual | absorve parte do dano do grupo

- 👁 Olhar da Sepultura | inimigos feridos devem testar Vontade ou ficam abalados

### Características de Classe:

> **Pontos de Vida: 16 PV. +5 PV por nível**

> **Pontos de Energia (PE): 3 por nível**

<!-- > **Perícias:
Vontade (Sab) e Intimidação (Car), mais 3 à escolha entre:
Religião (Sab), Percepção (Sab), Guerra (Int), Atletismo (For)** -->

> **Proficiências:
Armas marciais, escudos e armaduras pesadas**

---

## ❄ Glacial

- Base mitológica: frio primordial, batalhas em terras geladas, sobrevivência brutal.
- Função: Dano / Debuff
- Conceito: guerreiro que usa o frio como extensão da arma.

#### 📖 Habilidades:

- ❄ Corte Gélido | dano físico + aplica 1 Acúmulo de Frio
- 🧊 Armadura de Gelo Rachado | ganha defesa temporária
- 🌬 Inverno Persistente | inimigos próximos sofrem penalidade de ataque

`Ataques aplicam “Acúmulo de Frio” (3 acúmulos = alvo fica lento).`

### Características de Classe:

> **Pontos de Vida: 13 PV. +4 PV por nível**

> **Pontos de Energia (PE): 4 por nível**

<!-- > **Perícias:
Luta (For) e Sobrevivência (Sab), mais 3 à escolha entre:
Atletismo (For), Percepção (Sab), Intimidação (Car), Reflexos (Des)** -->

> **Proficiências:
Armas marciais, armaduras médias**

---

## 🌲 Vigia

- Base mitológica: florestas sagradas, espíritos naturais, criaturas selvagens.
- Função: Controle / Invocação leve
- Conceito: protetor das matas e dos espíritos antigos.

#### 📖 Habilidades:

- 🌿 Raízes Enredantes | prende inimigo no chão (teste de Reflexos)
- 🦌 Chamado Selvagem | invoca criatura espiritual
- 🍃 Sopro da Mata | cura leve em área

`Terrenos naturais concedem bônus extras.`

### Características de Classe:

> **Pontos de Vida: 11 PV. +3 PV por nível**

> **Pontos de Energia (PE): 5 por nível**

<!-- > **Perícias:
Sobrevivência (Sab) e Percepção (Sab), mais 3 à escolha entre:
Intuição (Sab), Religião (Sab), Atletismo (For), Diplomacia (Car)** -->

> **Proficiências:
Armas simples, armaduras leves**

---

## ⚒ Detonador

- Base mitológica: guerras, invasões, destruição de muralhas.
- Função: Dano Bruto / Anti-estrutura
- Conceito: especialista em quebrar formações e defesas.

#### 📖 Habilidades:

- 💥 Impacto Devastador | dano alto, chance de derrubar
- 🪨 Abalo de Guerra | atinge múltiplos inimigos próximos
- 🛡 Rompedor de Linhas | ignora parte da defesa

`Bônus contra inimigos em formação ou protegidos.`

### Características de Classe:

> **Pontos de Vida: 15 PV. +2 PV por nível**

> **Pontos de Energia (PE): 3 por nível**

<!-- > **Perícias:
Atletismo (For) e Guerra (Int), mais 3 à escolha entre:
Intimidação (Car), Luta (For), Percepção (Sab), Ofício (Int)** -->

> **Proficiências:
Armas pesadas, escudos, armaduras pesadas**

---

# 🧬 Raças:

## 🧑 Humanos

Humanos são descendentes de clãs guerreiros que sobreviveram a invernos rigorosos e guerras constantes. Adaptáveis e determinados, não possuem as habilidades extraordinárias de outras raças, mas compensam isso com versatilidade e ambição.
Vivem em vilarejos fortificados, navegando mares frios e explorando territórios hostis. Para eles, honra e reputação são tão valiosas quanto ouro.
Humanos costumam se destacar em qualquer classe, moldando-se conforme a necessidade do grupo.

#### 📖 Habilidades de Raça:

***Poder 10, Defesa 5, Vida +1, Esquiva 1***

- Adaptabilidade.
> Uma vez por descanso, pode rerrolar um teste de atributo.

- Versatilidade Natural.
> Ganha +5 em atributos (Poder, Defesa, Vida, Esquiva).

- Tenacidade.
> Uma vez por descanso, ao cair a 0 PV, permanece com 1 PV.

Deslocamento: 9m

Longevidade: Até 80 anos.

---

## 🧔🏻 Anões do Norte

Anões do Norte são mestres da forja e da resistência. De corpo robusto e barba espessa, vivem em fortalezas esculpidas na pedra das montanhas congeladas.
Valorizam tradição, honra e habilidade artesanal. Entre anões, armas são obras de arte e juramentos são eternos.
São naturalmente resistentes e difíceis de derrubar.

#### 📖 Habilidades de Raça:

***Poder  12, Defesa 10, Vida +2, Esquiva 3***

- Resistência Ancestral.
> Resistência a veneno.

- Filho da Forja.
> Bônus em testes relacionados a criação e reparo de equipamentos.

- Tenacidade.
> Uma vez por descanso, ao cair a 0 PV, permanece com 1 PV.

Deslocamento: 9m

Longevidade: Até 250 anos.

---

## 🧝 Elfos Crepusculares

Elfos são seres ligados à natureza e às energias sutis do mundo. De aparência esguia e traços delicados, possuem sentidos aguçados e forte conexão espiritual com o ambiente ao seu redor.
Vivem em florestas antigas ou regiões isoladas, onde preservam tradições mágicas e conhecimentos ancestrais.
São naturalmente ágeis e inclinados à magia.

#### 📖 Habilidades de Raça:

***Poder 12, Defesa 7, Vida +2, Esquiva 2***

- Visão na Penumbra.
> Enxerga normalmente em ambientes com pouca luz.

- Afinidade Mística.
> Redução no custo de habilidades mágicas básicas.

- Sentidos Aguçados.
> Ganha +4 em Poder.

Deslocamento: 9m

Longevidade: Até 180 anos.

---

## 🧊 Jotunn

Descendentes de gigantes ancestrais das regiões geladas, Jotunn são imponentes e fisicamente superiores à maioria das outras raças.
Sua pele pode ter tons pálidos ou azulados, e seus olhos lembram gelo rachado. Muitos vivem isolados, mas alguns buscam provar seu valor entre outros povos.
São naturalmente fortes e resistentes ao frio.

#### 📖 Habilidades de Raça:

***Poder 20, Defesa 7, Vida +2, Esquiva 0***

- Sangue Glacial.
> Resistência a dano de frio.

- Presença Intimidante.
> Ganha +1 em Esquiva.

- Estrutura Massiva.
> Ganha +3 em Defesa.

Deslocamento: 9m

Longevidade: Semelhante à humana.

---

## 👻 Draugr

Draugr são mortos que retornaram com consciência e vontade própria. Diferente de mortos-vivos comuns, mantêm personalidade e memória de suas vidas passadas.
Sua aparência é pálida, com traços rígidos e olhos sem brilho. São temidos e muitas vezes rejeitados, mas podem ser aliados poderosos.
Possuem resistência sobrenatural, mas também carregam o peso da morte.

#### 📖 Habilidades de Raça:

***Poder 10, Defesa 15, Vida +3, Esquiva 1***

- Corpo Inerte.
> Resistência a dano físico não mágico.

- Natureza Incansável.
> Não precisa respirar nem comer.

- Presença Sombria.
> Ganha +3 em vida.

- Vulnerabilidade Sagrada.
> Caso o inimigo acerte um crítico, sofre o triplo do dano.

Deslocamento: 9m

Longevidade: Indefinida.

---

## 🐺 Metamorfo

Metamorfos são humanos com sangue bestial em suas veias. Diz-se que seus ancestrais vestiam peles de feras em rituais antigos e jamais abandonaram completamente essa conexão.
Em momentos de tensão, seus instintos despertam, revelando presas afiadas, olhos selvagens e força ampliada.
Vivem entre humanos, mas raramente são totalmente aceitos.

#### 📖 Habilidades de Raça:

***Poder 15, Defesa 7, Vida +1, Esquiva 2***

- Forma Bestial.
> Uma vez por combate, pode assumir forma parcial bestial por alguns turnos, recebendo +3 em Poder.

- Instinto Predador.
> Procura por rotas de fuga (+3 em Esquiva).

Deslocamento: 9m

Longevidade: Semelhante à humana.

---

## 🤝 Autores
#### *Pedro Henrique Nascimento da Silva, Lucas Tadashi Azambuja Miasaki, Henry Youji Oseki*

## 📝 Licença

Este projeto está sob a licença MIT. Veja o arquivo LICENSE para mais detalhes.
